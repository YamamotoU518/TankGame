...
void gameStart() {
    ...
    camera->setPosition({ 0, 100, -700 });

    // ( テスト用 ) カヌー作成
    canoe = dxe::Mesh::CreateFromFileMV("resource/mesh/mv/canoe/Canoe.mv1");
    canoe->setScaling({ 10, 10, 10 });
    canoe->setPosition({ 100, 3, -375 });

    // ( テスト用 ) 桟橋作成
    bridge = dxe::Mesh::CreateFromFileMV("resource/mesh/mv/bridge/WoodenBridge_01.mv1");
    ...
    skybox->setTexture(dxe::Texture::CreateFromFile("resource/graphics/example/skybox/skybox_b.png"));
    skybox->loadMaterial("resource/dxe_parameters/mesh_material/material.bin");

    // ( テスト用 ) 砂浜作成
    ground = dxe::Mesh::CreatePlaneMV({ 1000, 300, 0 });
    ground->setTexture(dxe::Texture::CreateFromFile("resource/graphics/example/sandy_beach.png"));
    ...
    test_object->setTexture(dxe::Texture::CreateFromFile("resource/graphics/example/test.jpg"));
    test_object->setPosition({ 300, -100, 100 });


    // 水面オブジェクトをデータから作成
    water = std::make_shared<dxe::WaterPlane>("resource/dxe_parameters/water_plane/water_plane.bin");
    ...
    // ( テスト用 ) パーティクル作成
    ptcl = std::make_shared<dxe::Particle>("resource/dxe_parameters/particle/preset/fire.bin");
    ...
    origine_mesh = dxe::Mesh::CreateFromFileMV("resource/mesh/mv/preset/cube.mv1", 25);
    origine_mesh->setTexture(dxe::Texture::CreateFromFile("resource/graphics/example/test.jpg"));


    std::vector<Shared<dxe::Texture>> textures;
    textures.emplace_back(dxe::Texture::CreateFromFile("resource/graphics/example/red1.bmp"));
    textures.emplace_back(dxe::Texture::CreateFromFile("resource/graphics/example/blue1.bmp"));
    textures.emplace_back(dxe::Texture::CreateFromFile("resource/graphics/example/green1.bmp"));
    ...
    for (int i = 0; i < mesh_pool->getMaxInstanceNum(); ++i) {
        ...
        inst->setPosition({ float(i * 50), 50, float(i * 50) });
...
void gameMain(float delta_time) {
    ...
    // キューブマップへの書き込み処理
    //
    tnl::Vector3 cube_map_cam_pos = camera->getPosition() * tnl::Vector3(1, -1, 1);
    ...
    dxe::DrawFpsIndicator({ DXE_WINDOW_WIDTH - 310, 0 });
