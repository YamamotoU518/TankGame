...
void gameMain(float delta_time) {
	...
	//
	{
		...
		DrawFormatString(0, 0, -1, "振幅は 0 スタートで 1.0f ～ -1.0 まで振れ 0 に収束");
		DrawFormatString(0, 20, -1, "1 キーで時間リセット");
		DrawFormatString(0, 40, -1, "%f", osc);
	}
	...
	//
	{
		...
		DrawFormatString((int)scr_ofs_x, 0, -1, "振幅は 0 スタートで 0.0f ～ 2.0 まで振れ 1 に収束");
		DrawFormatString((int)scr_ofs_x, 20, -1, "2 キーで時間リセット");
		DrawFormatString((int)scr_ofs_x, 40, -1, "%f", osc);
	...
	//
	{
		...
		DrawFormatString(0, scr_ofs_y + 0, -1, "振幅は 0 スタートで 1.0f ～ -1.0 まで振れ 0 に収束");
		DrawFormatString(0, scr_ofs_y + 20, -1, "3 キーで時間リセット");
		DrawFormatString(0, scr_ofs_y + 40, -1, "%f", osc);
	}
	...
	//
	{
		...
		int scr_ofs_y = DXE_WINDOW_HEIGHT * 0.5f;
		DrawFormatString(scr_ofs_x, scr_ofs_y + 0, -1, "振幅は 0 スタートで 0.0f ～ 2.0 まで振れ 1 に収束");
		DrawFormatString(scr_ofs_x, scr_ofs_y + 20, -1, "マウスで玉を引っ張りましょう");
		DrawFormatString(scr_ofs_x, scr_ofs_y + 40, -1, "%f", osc);
	}
