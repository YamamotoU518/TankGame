...
void gameMain(float delta_time) {
	...
	DrawFormatString(0, 0, -1, "WASD で A を移動");
	DrawFormatString(0, 30, -1, "カーソルキー で B を移動");
	...
	if (tnl::IsIntersectRect(posA, sizeA.x, sizeA.y, posB, sizeB.x, sizeB.y)) {
		...
		DrawFormatString(0, 60, -1, "%s", s[(int)n].c_str());
	}


	dxe::DrawBox({ posA.x, posA.y }, sizeA.x, sizeA.y, false, -1);
