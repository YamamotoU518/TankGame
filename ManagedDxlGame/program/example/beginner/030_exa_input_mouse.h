...
void gameMain(float delta_time) {
	...
	DrawFormatString(0, 0, -1, "左クリック押しっぱなし(Z) %d", on_keep_count_up_value);

	DrawFormatString(0, 30, -1, "右クリック押した瞬間(X) %d", on_trigger_count_up_value);

	DrawFormatString(0, 60, -1, "マウス座標 [ x = %.2f ] [ y = %.2f ]", mouse_position.x, mouse_position.y);
