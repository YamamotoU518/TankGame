...
void gameMain(float delta_time) {
	...
	DrawFormatString(0, 0, -1, "□ (A) 押しっぱなし %d", on_keep_count_up_value);
	DrawFormatString(0, 30, -1, "× (B) 押した瞬間 %d", on_trigger_count_up_value);
	DrawFormatString(0, 60, -1, "左スティック軸 [ axis x = %.2f ] [ axis y = %.2f ]", l_stick_axis_value.x, l_stick_axis_value.y);
	DrawFormatString(0, 90, -1, "右スティック軸 [ axis x = %.2f ] [ axis y = %.2f ]", r_stick_axis_value.x, r_stick_axis_value.y);
