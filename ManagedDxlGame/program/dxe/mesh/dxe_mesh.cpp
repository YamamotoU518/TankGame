...
namespace dxe {
	...
	void Mesh::copySetting(Shared<Mesh> other) {
		...
		other->mv1_base_scale_ = mv1_base_scale_;
	}
	...
	bool Mesh::exportForFileFormatMV(const std::string& file_path) {
		if (mesh_format_ == eMeshFormat::MESH_FMT_PG) {
			Shared<Mesh> mesh;
			...
			MV1SaveModelToMV1File(mv_hdl_, file_path.c_str());
		}
		return true;
	}
	...
	//----------------------------------------------------------------------------------------
	Shared<Mesh> Mesh::CreateFromFileMV(const std::string& file_path, const float base_scl)
	{
		Shared<Mesh> mesh = Shared<Mesh>(new Mesh());
		mesh->shape_type_ = eShapeType::RESOUCE;
		mesh->mesh_format_ = Mesh::eMeshFormat::MESH_FMT_MV;
		mesh->mv_hdl_ = MV1LoadModel(file_path.c_str());
		mesh->mv1_base_scale_ = base_scl;
		mesh->desc_ = std::make_shared<dxe::MeshDescResouce>( file_path, 1.0f );
		DxLib::VECTOR dxv = MV1GetMeshMaxPosition(mesh->mv_hdl_, 0);
		mesh->bd_box_size_ = tnl::Vector3(dxv.x, dxv.y, dxv.z) * 2.0f ;
		mesh->bd_sphere_radius_ = mesh->bd_box_size_.length() * 0.5f ;
	...
	void Mesh::render(const Shared<dxe::Camera> camera) {
		...
		mt_scale = tnl::Matrix::Scaling(scl_.x * mv1_base_scale_, scl_.y * mv1_base_scale_, scl_.z * mv1_base_scale_);
		...
		DxLib::VECTOR base_scale = { mv1_base_scale_, mv1_base_scale_ , mv1_base_scale_ };
