...
namespace dxe {
	...
	public :
		...
		// arg1... フィルパス
		// arg2... スケール( デフォルト1.0f )
		static std::vector<Shared<Mesh>> CreateFromFileObjMV(const std::string& file_path, const float base_scl = 1.0f);

		//-----------------------------------------------------------------------------------------------------
		// DXLib が対応している形式ファイルからメッシュを作成
		// arg1... フィルパス
		// arg2... スケール( デフォルト1.0f )
		static Shared<Mesh> CreateFromFileMV(const std::string& file_path, const float base_scl = 1.0f);
