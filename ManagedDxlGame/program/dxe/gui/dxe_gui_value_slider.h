	...
	public :
		...
		void draw() {
			...
			if constexpr (GuiValueSliderTraits<int>::Value == GuiValueSliderTraits<U>::Value) {
				DrawFormatString(pos_.x + slider_lenght_ + TAB_SIZE_W2 + 5, pos_.y - TAB_SIZE_H2 + 3, color, "%d", accessor_.get());
			}
			if constexpr (GuiValueSliderTraits<float>::Value == GuiValueSliderTraits<U>::Value) {
				DrawFormatString(pos_.x + slider_lenght_ + TAB_SIZE_W2 + 5, pos_.y - TAB_SIZE_H2 + 3, color, "%.3f", accessor_.get());
			}
			if (is_description_ && description_function_) color = -1;
			DrawFormatString(pos_.x + slider_lenght_ + num_space, pos_.y - TAB_SIZE_H2 + 3, color, "%s", label_.c_str());
		}
	...
	public:
		...
		void draw() {
			...
			for (int i = 0; i < 2; ++i) {
				...
				DrawString((float)pos_.x + (float)slider_lenght_ + TAB_SIZE_W2 + 5, (float)pos.y - TAB_SIZE_H2 + 3, color, "%d", param[i] );
				DrawString((float)pos_.x + (float)slider_lenght_ + num_space, (float)pos.y - TAB_SIZE_H2 + 3, color, "%s", ( label_ + spram[i] ).c_str() );
			}
		}
	...
	public:
		...
		void draw() {
			...
			for (int i = 0; i < 3; ++i) {
				...
				DrawLine(pos_.x + slider_lenght_, pos.y - TAB_SIZE_H2, pos_.x + slider_lenght_, pos.y + TAB_SIZE_H2, color);
				DrawBox(tab_pos_[i].x - TAB_SIZE_W2, tab_pos_[i].y - TAB_SIZE_H2, tab_pos_[i].x + TAB_SIZE_W2, pos.y + TAB_SIZE_H2, color, false);
				DrawFormatString(pos_.x + slider_lenght_ + TAB_SIZE_W2 + 5, pos.y - TAB_SIZE_H2 + 3, color, "%.3f", param[i]);
				if (is_description_ && description_function_) color = -1;
				DrawFormatString(pos_.x + slider_lenght_ + num_space, pos.y - TAB_SIZE_H2 + 3, color, "%s", (label_ + spram[i]).c_str());
			}
		}
